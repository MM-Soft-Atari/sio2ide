...
static void LISTV_Draw( T_listview * listv, INT16 pos, UINT16 maxcap, UINT8 maxx, UINT8 maxy, BOOL refr )
{
  static UINT8 oldpos = 0;
  UINT8      * itm;
  INT16        i;
  UINT8        x, y;
  UINT8        ad;
  ...
  {
    i = (oldpos % maxcap) / maxy;                       // column
    x = listv->hx + (i * (listv->itmlen + ad)) + 1;     // x
    y = listv->hy + ((oldpos % maxcap) % maxy) + 1;     // y
    gotoxy( x, y );
    if( listv->gitem( &itm, oldpos ) )
    {
      LISTV_DrawItem( itm, listv->itmlen, FALSE );
    }
    //
    i = (pos % maxcap) / maxy;                          // column
    x = listv->hx + (i * (listv->itmlen + ad)) + 1;     // x
    y = listv->hy + ((pos % maxcap) % maxy) + 1;        // y
    gotoxy( x, y );
    if( listv->gitem( &itm, pos ) )
    {
      LISTV_DrawItem( itm, listv->itmlen, TRUE );
    }
    //
    oldpos = pos;
  }
...
UINT8 LISTV_Show( T_listview * listv )
{
  ...
  for(;;)
  {
    if( curpag != curpos / maxcap )
    {
      LISTV_Draw( listv, curpos, maxcap, maxx, maxy, TRUE );
      curpag = curpos / maxcap;
    }
    else
    {
      LISTV_Draw( listv, curpos, maxcap, maxx, maxy, FALSE );
    }
    if( listv->focus )
    {
      listv->focus( curpos );
    ...
    switch( LISTV_WaitKey( listv, &key ) )
    {
      ...
      case 0xFE:        // Action
        if( listv->action )
          if( listv->action( (UINT16*)&curpos, key, &refr ) )
          {
            return key;
          }
        if( refr || (curpag != curpos / maxcap) )
        {
          curpag = 0xFF;
          LISTV_ReFresh( listv );
        }
