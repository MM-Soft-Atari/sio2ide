...
#define SIO_TSTOP       0xFFFE          // STOP TimeOut Timer
...
#define TOIE0           1       // Timer0 Ovf Int Enable Bit
//
//      SIO Answer codes
//
...
STATIC DATA VOLATILE UINT16    sioBPtr;             // Buffer Data Pointer
STATIC DATA VOLATILE UINT16    sioBDLen;            // Buffer Data Length
STATIC DATA VOLATILE UINT8     sioChk;              // Checksum
//
STATIC DATA VOLATILE UINT8     sioStat;             // Current state
STATIC DATA VOLATILE UINT8     sioCmd;              // Current command
STATIC DATA VOLATILE UINT16    sioTOut;             // TimeOut Timer
...
{
  if( __inp( UBRR ) == SIORATE_FACT_L )
  {
    __outp( UBRR, SIORATE_FACT_H );
  }
  else
  {
    __outp( UBRR, SIORATE_FACT_L );
  }
}
...
{
  __port_and( UCR, ~(1<<RXCIE) );                           //
  sioBDLen = len;
  sioBPtr = 0;
  SIO_ChkClr();
  __port_or( UCR, (1<<RXCIE) );                            //
}
...
STATIC VOID SIO_TOutSet( UINT16 tout )
{
...
VOID   SIO_Init( VOID )
{
  ...
  __outp( UBRR, SIORATE_FACT_L );                  // Baud Rate factor
  __outp( UCR, ((1<<RXCIE) + (1<<RXEN) + (1<<TXEN)) );
  //
...
{
  loop_until_bit_is_set( USR, UDRE );   // wait for TX empty
  __outp( UDR, c );
}
...
//----------------------------------------------------------------
#ifdef __GNU__
STATIC VOID progmem_read( VOID * dest, CONST prog_char * src, size_t n )
{
  while( n-- )
  {
    *(char *)dest++ = PRG_RDB( src++ );
  }
}
#endif
...
#ifdef __IAR__
interrupt [UART_RX_vect] VOID SIO_RXint (VOID)
#endif
#ifdef __GNU__
...
#ifdef __IAR__
interrupt [TIMER0_OVF0_vect] VOID TC0_Ovf (VOID)
#endif
#ifdef __GNU__
