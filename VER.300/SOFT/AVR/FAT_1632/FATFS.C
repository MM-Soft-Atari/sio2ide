...
//
STATIC       UINT8      secBuff[512];   // Sector buffer
STATIC       T_disktab  diskTab;        // ATR table
STATIC       T_drive    cDrive;         // Drive logical info
STATIC       T_drvinf   drvInf;         // Drive phisical info
//
...
STATIC BOOL FindFileSect( T_file * file, T_fldet * fdet )
{
  if( GetFirstFile_Virtual( file, TestATRFile ) )
  {
    do
    {
      if( file->flDet.flStartSec == fdet->flStartSec )
      {
        return TRUE;
      }
    }while( GetNextFile_Virtual( file, TestATRFile ) );
...
#ifdef FATFS_TEST
  if( debug ) Printf(    "\n  MBR mark       : 0x%x%x", (mbr->mbrEnd>>8)&0xFF, mbr->mbrEnd&0xFF );
#endif

  if( mbr->mbrEnd == MBRENDMARK )
  {
    prt = &mbr->mbrPTable[0];
...
{
  return EE_PutSavedDir( &cDrive.cwdirStartSec );
}
...
BOOL FATFS_DiskGet( UINT8 id, T_file * disk )
{
  ...
  if( (id > 0) && (id <= MAX_FL_ID) )
  {
    if( diskTab[ CFG_FL_ID ].flStat == FLS_CFGFILE )
    {
      if( diskTab[ id ].flStat & FLS_FILEOK )
      {
        if( FindFileSect( disk, &diskTab[ id ] ) )
        {
...
BOOL FATFS_DiskSet( UINT8 id, T_file * disk )
{
  ...
  if( (id > 0) && (id <= MAX_FL_ID) )
  {
    if( diskTab[ CFG_FL_ID ].flStat == FLS_CFGFILE )
    {
      if( FindFileSect( &file, &disk->flDet ) )
      {
        dsec = find.HD.csect;
        //
        if( TestFileStruct( &file.flDet ) )
...
BOOL FATFS_SaveConfig( VOID )
{
  ...
  if( FindFileNameF( &file, cfgFileName ) )
  {
    ...
    for( id = 1; id <= MAX_FL_ID; id++ )
    {
      ...
      if( diskTab[ id ].flStat & FLS_FILEOK )
      {
        if( FindFileSect( &file, &diskTab[ id ] ) )
        {
          //
...
BOOL FATFS_GetLFN( UINT8 * name, UINT8 * lfn )
{
  ...
  // FAT16 FAT32
  //
  if( FindFileName( &file, name ) )
  {
    if( find.HD.cent->deName[6] == '~' )
    {
  ...
  if( (disk > 0) && (disk <= MAX_FL_ID) )
  {
      ...
      )
    {
      ...
      if( (ssize == 128) || (sec >= 1 && sec <= 3) )
      {
        ...
        //
        sps   = SECTOR_SIZE / 128;
        snum  = diskTab[ disk ].flStartSec + (((UINT32)sec - 1) / (UINT32)sps);
        //
        if( (sec % sps) != 0 )
        {
          if( !SectorRead( snum ) )
      ...
      else if( ssize == 256 )
      {
        ...
        //
        sps   = SECTOR_SIZE / 256;
        snum  = diskTab[ disk ].flStartSec + (((UINT32)sec - 3) / (UINT32)sps);
        //
        if( (sec % sps) != 0 )
        {
          if( !SectorRead( snum ) )
    ...
    if( file->flDet.flSize == 0 )
    {
      ...
      if( (ptr = strtokf( secBuff, cfgTokenDel, &tok )) != NULL )
      {
        ...
        for( id = 1; id <= MAX_FL_ID; id++ )
        {
          ...
          else if( diskTab[ id ].flStat & FLS_NOTDEFR )
          {
            Printf( "     %.8s.ATR  ", file->flName );
            Printf( "%05u x %sB         File NOT defragmented. ",
                    diskTab[ id ].flSize,
                    diskTab[ id ].flStat & FLS_SEC256B ? "256" : "128" );
          }
          else
...
BOOL FATFS_Init( VOID )
{
  ...
  //
  if( EE_GetSavedDir( &cDrive.cwdirStartSec ) )
  {
#ifdef DEBUG_FAT
    Printf( "\nIS" );
#endif
    ...
    {
      EE_PutSavedDir( &cDrive.rdirStartSec );
    }
  }
  else
  {
